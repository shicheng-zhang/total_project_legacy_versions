...
#include <stdlib.h>
#include <stdio.h>
#define MAX_POOL_CAPACITY 16384
static rigidbody object_memory_pool [MAX_POOL_CAPACITY];
static bool pool_is_initialized = false;
void scene_allocate_pool (void) {
    if (!pool_is_initialized) {
        obj_per_scene = object_memory_pool;
        object_capacity = MAX_POOL_CAPACITY;
        pool_is_initialized = true;
    }
} int scene_add_object (float radius, float mass, vector3 initial_position) {
    scene_allocate_pool ();
    if (object_count >= MAX_POOL_CAPACITY) { fprintf (stderr, "Error POOL01: Maximum object capacity reached.\n"); return -1; }
