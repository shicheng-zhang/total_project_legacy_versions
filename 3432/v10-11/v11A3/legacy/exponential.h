...
static inline double logarithm_base(double x, double b) { return ml_log(x) / ml_log(b); }
static inline double hyperbolic_sine(double x) { return (ml_exp(x) - ml_exp(-x)) / 2.0; }
static inline double hyperbolic_cosine(double x) { return (ml_exp(x) + ml_exp(-x)) / 2.0; }
static inline double hyperbolic_tangent(double x) { return hyperbolic_sine(x) / hyperbolic_cosine(x); }
static inline double inverse_hyperbolic_sine(double x) { return ml_log(x + ml_sqrt(x * x + 1.0)); }
static inline double inverse_hyperbolic_cosine(double x) { return (x < 1.0) ? 0.0/0.0 : ml_log(x + ml_sqrt(x * x - 1.0)); }
static inline double inverse_hyperbolic_tangent(double x) { return (x <= -1.0 || x >= 1.0) ? 0.0/0.0 : 0.5 * ml_log((1.0 + x) / (1.0 - x)); }
